...
def run_calibration():
    ...
    for h in HORIZONS:
        ...
        mask = y_full.notna() & (X_full.index >= cal_start) & (X_full.index <= cal_end)
        X_cal = X_full[mask]
        y_cal = y_full[mask]
        ...
        for season, month_set in [("summer", SUMMER_MONTHS), ("winter", WINTER_MONTHS)]:
            ...
            raw_cov = float(
                np.mean(
                    (true_abs[mask_s] >= pred_lo[mask_s]) & (true_abs[mask_s] <= pred_hi[mask_s])
                )
            )
            q = compute_conformal_scale(true_abs[mask_s], pred_lo[mask_s], pred_hi[mask_s], ALPHA)
            cal_cov = float(
                np.mean(
            ...
            log.info(
                "  %s (n=%d): raw_cov=%.1f%%  q=%.3f  cal_cov=%.1f%%  width %.1f→%.1f",
                season,
                n_s,
                raw_cov * 100,
                q,
