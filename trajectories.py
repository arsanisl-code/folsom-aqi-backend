"""
trajectories.py — Lagrangian Back-Trajectory Kinematics.

Computes the estimated origin coordinates of air parcels arriving at the target location
...
Physics (Euler step-back):
    ...
    origin_lat = DEFAULT_LAT + Δlat
    origin_lon = DEFAULT_LON + Δlon
...
KM_PER_DEG_LON: float = 111.32 * np.cos(np.radians(DEFAULT_LAT))
...
def add_trajectory_features(
    ...
    for h in TRAJ_LOOKBACKS:
        ...
        # Euler step-back: origin = target - mean_wind × dt
        # Convert m/s × seconds → km → degrees
        delta_lat = -(V_mean * dt_s / 1000.0) / KM_PER_DEG_LAT
        delta_lon = -(U_mean * dt_s / 1000.0) / KM_PER_DEG_LON

        X[f"traj_origin_lat_{h}h"] = DEFAULT_LAT + delta_lat
        X[f"traj_origin_lon_{h}h"] = DEFAULT_LON + delta_lon
