"""
train_ensemble.py —  Feature-Diversity Stacked Ensemble for Folsom AQI Navigator.
...
def _read_report() -> dict:
    ...
    return {
        ...
        "architecture": " Feature-Diversity Ensemble (LGBM-Full + XGB + LGBM-Physics + NNLS-Bias)",
...
def _lgbm_full_objective(trial, X_tr, y_tr, X_va, y_va, warmstart: dict, timer: _TrialTimer):
    ...
    LGBM-Full objective — regression_l1 (MAE) loss, all features.
     change: huber → regression_l1 to align training loss with eval metric.
    """
...
class _NNLSMeta:
    ...
    : OOF matrix is augmented with a column of ones before solving:
...
def train_ensemble_horizon(df: pd.DataFrame, horizon_h: int, val_cutoff: datetime) -> dict:
    """Full  ensemble pipeline for one horizon."""
    log.info("=" * 65)
    ...
    # ── 5. HPO eval set — fold 5 of TimeSeriesSplit ( fix) ────────────
    ...
    # ── 6. Optuna HPO ─────────────────────────────────────────────────────
    #  Tier 2: Increased HPO budget (50→150, 75→200, 75→150).
    ...
    # ── 9. Final base model training (ES_DAYS=60,  fix) ───────────────
    ...
    result = {
        ...
        "xgb_": _m("XGBoost ", pred_xgb),
        "lgbm_physics_": _m("LGBM-Physics ", pred_lgbm_physics),
        "ensemble_": _m("★ ENSEMBLE ", pred_meta),
        "meta_coefficients": {
...
def _print_tournament_table(report: dict) -> None:
    log.info("")
    log.info("=" * 80)
    log.info("   FEATURE-DIVERSITY ENSEMBLE — TOURNAMENT REPORT")
    log.info("=" * 80)
    ...
    for hr in report.get("horizons", []):
        ...
        rows = [
            ...
            ("LGBM-Full ", hr.get("lgbm_full_", {})),
            ("XGBoost ", hr.get("xgb_", {})),
            ("LGBM-Physics ", hr.get("lgbm_physics_", {})),
            ("★ ENSEMBLE ", hr.get("ensemble_", {})),
        ]
        for name, m in rows:
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=" Feature-Diversity Ensemble")
    parser.add_argument(
        "--horizon",
        type=int,
        choices=[6, 12, 24, 48],
...
def main() -> None:
    ...
    log.info("=" * 65)
    log.info("  Folsom AQI Navigator —  Feature-Diversity Ensemble")
    ...
    # Train
    log.info("Step 2: Training  ensemble: %s", horizons_to_run)
    for h in horizons_to_run:
        train_ensemble_horizon(df, h, val_cutoff)

    # Feature names
    feature_names = get_feature_names(6)
    (MODELS_DIR / "feature_names_.json").write_text(json.dumps(feature_names, indent=2))
    ...
    log.info(" complete.  Total wall time: %.1fs (%.1f min)", total_run_s, total_run_s / 60)
