...
class _TrialTimer:
    ...
    def __call__(self, study: optuna.Study, trial: optuna.trial.FrozenTrial) -> None:
        ...
        log.info(
            ...
            self.label, trial.number + 1, self.n_trials,
...
def _lgbm_full_objective(trial, X_tr, y_tr, X_va, y_va,
                         warmstart: dict, timer: _TrialTimer):
    """
...
def _lgbm_physics_objective(trial, X_tr_phys, y_tr, X_va_phys, y_va,
                             timer: _TrialTimer):
    """
...
def run_lgbm_full_hpo(X_tr, y_tr, X_va, y_va, horizon_h: int,
                      n_trials: int = 50) -> tuple[dict, dict]:
    warmstart  = _load_v9_warmstart(horizon_h)
    study_name = f"lgbm_full_{horizon_h}h"
    log.info("  [LGBM-Full HPO] study=%s  trials=%s  warmstart=%d keys",
             study_name, n_trials, len(warmstart))

    timer = _TrialTimer(f"LGBM-Full-{horizon_h}h", n_trials=n_trials)
    study = optuna.create_study(
        study_name=study_name, storage=OPTUNA_DB, load_if_exists=True,
        direction="minimize",
    ...
    study.optimize(
        ...
        n_trials=remaining, callbacks=[timer], show_progress_bar=False,
    )
    wall = time.perf_counter() - t0
    timing = timer.summary()
    log.info("  [LGBM-Full HPO] done  best=%.4f  wall=%.1fs  %.1f trials/min",
             study.best_value, wall, timing.get("trials_per_min", 0))
    return study.best_params, timing


def run_xgb_hpo(X_tr, y_tr, X_va, y_va, horizon_h: int,
                n_trials: int = 75) -> tuple[dict, dict]:
    ...
    wall = time.perf_counter() - t0
    timing = timer.summary()
    log.info("  [XGB HPO] done  best=%.4f  wall=%.1fs  %.1f trials/min",
             study.best_value, wall, timing.get("trials_per_min", 0))
    return study.best_params, timing


def run_lgbm_physics_hpo(X_tr_phys, y_tr, X_va_phys, y_va, horizon_h: int,
                          n_trials: int = 75) -> tuple[dict, dict]:
    study_name = f"lgbm_physics_{horizon_h}h"
    log.info("  [LGBM-Physics HPO] study=%s  trials=%s  features=%d",
             study_name, n_trials, X_tr_phys.shape[1])
    ...
    remaining = max(0, n_trials - len(study.trials))
    log.info("  [LGBM-Physics HPO] %d done, running %d more",
             n_trials - remaining, remaining)
    ...
    wall = time.perf_counter() - t0
    timing = timer.summary()
    log.info("  [LGBM-Physics HPO] done  best=%.4f  wall=%.1fs  %.1f trials/min",
             study.best_value, wall, timing.get("trials_per_min", 0))
    return study.best_params, timing
...
def build_lgbm_full(best_params: dict, X_fit, y_fit,
                    X_es, y_es) -> lgb.LGBMRegressor:
...
def build_xgb(best_params: dict, X_fit, y_fit,
              X_es, y_es) -> XGBRegressor:
...
def build_lgbm_physics(best_params: dict, X_fit_phys, y_fit,
                       X_es_phys, y_es) -> lgb.LGBMRegressor:
...
def generate_oof_predictions(
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(tscv.split(X_arr)):
        ...
        lgbm_f = lgb.LGBMRegressor(
            ...
            n_jobs=N_JOBS, verbosity=-1, random_state=42, bagging_freq=1,
            **lgbm_full_params,
        )
        lgbm_f.fit(
            X_fit_f, y_fit_f,
        ...
        xgb_f = XGBRegressor(
            ...
            n_jobs=N_JOBS, verbosity=0, random_state=42, tree_method="hist",
        ...
        phys_f = lgb.LGBMRegressor(
            objective="regression_l1", n_estimators=2000,
            n_jobs=N_JOBS, verbosity=-1, random_state=99, bagging_freq=1,
            **lgbm_physics_params,
        )
        phys_f.fit(
            X_fit_phys_f, y_fit_f,
...
def train_ensemble_horizon(df: pd.DataFrame, horizon_h: int,
                           val_cutoff: datetime) -> dict:
    ...
    t0 = time.perf_counter()
    # V12: extract firms_hourly for trajectory features
    _firms_cols = ['fire_frp_raw', 'fire_count_raw', 'fire_min_dist_raw', 'fire_bearing_nearest']
    _avail = [c for c in _firms_cols if c in df.columns]
    _firms_hourly = df[_avail][df['fire_frp_raw'] > 0].copy() if _avail and 'fire_frp_raw' in df.columns else pd.DataFrame()
    X_full, y_full = engineer_features(df, horizon_h, firms_hourly=_firms_hourly)
    mask = y_full.notna()
    X_full, y_full = X_full[mask], y_full[mask]
    log.info("  Features: %s rows × %s cols  (%.1fs)",
             f"{len(X_full):,}", len(X_full.columns), time.perf_counter() - t0)

    # ── 2. Physics column selection ───────────────────────────────────────
    physics_cols = _get_physics_cols(X_full)
    log.info("  Physics features available: %d / %d requested",
             len(physics_cols), len(PHYSICS_FEATURES))
    ...
    log.info("  HPO fold 5: train=%d  val=%d  physics_features=%d",
             len(X_hpo_tr), len(X_hpo_va), len(physics_cols))
    ...
    lgbm_physics_params, lgbm_physics_timing = run_lgbm_physics_hpo(
        X_hpo_tr_phys, y_hpo_tr, X_hpo_va_phys, y_hpo_va,
        horizon_h, n_trials=150)
    ...
    (MODELS_DIR / f"hpo_params_{horizon_h}h.json").write_text(
        json.dumps(hpo_results, indent=2))
    ...
    log.info(
        ...
        meta.coef_[0], meta.coef_[1], meta.coef_[2], meta.bias_,
        meta.coef_.sum(),
    )
    ...
    lgbm_full_final = build_lgbm_full(lgbm_full_params, X_fit_df, y_fit,
                                      X_es_df, y_es_arr)
    ...
    lgbm_physics_final = build_lgbm_physics(
        lgbm_physics_params,
        X_fit_df[physics_cols], y_fit,
        X_es_df[physics_cols],  y_es_arr,
    )
    t_phys_s = round(time.perf_counter() - t3, 2)

    log.info("  Build times — LGBM-Full=%.1fs  XGB=%.1fs  LGBM-Physics=%.1fs",
             t_lgbm_s, t_xgb_s, t_phys_s)
    ...
    (MODELS_DIR / f"physics_cols_{horizon_h}h.json").write_text(
        json.dumps(physics_cols, indent=2))
    # Sentinel LAST
    ...
    _upsert_horizon_result(result)
    log.info("  Horizon %sh complete  wall=%.1fs (%.1f min)",
             horizon_h, horizon_wall_s, horizon_wall_s / 60)
    return result
...
def _print_tournament_table(report: dict) -> None:
    ...
    for hr in report.get("horizons", []):
        ...
        log.info(
            ...
            h, coef.get("lgbm_full", 0), coef.get("xgb", 0),
            coef.get("lgbm_physics", 0), coef.get("bias", 0),
            coef.get("sum", 0),
        )
    ...
    log.info("  %-8s %-26s %-14s %-10s %-10s",
             "Horizon", "Study", "Trials/min", "Mean(s)", "Total(s)")
    log.info("  " + "-" * 72)
    for hr in report.get("horizons", []):
        h  = hr["horizon_h"]
        bm = hr.get("benchmark_9800x3d", {})
        for key, label in [("lgbm_full_hpo",    "LGBM-Full HPO"),
                            ("xgb_hpo",          "XGBoost HPO"),
                            ("lgbm_physics_hpo", "LGBM-Physics HPO")]:
            t = bm.get(key, {})
            if not t:
                continue
            log.info("  %-8s %-26s %-14.1f %-10.1f %-10.1f",
                     f"{h}h", label,
                     t.get("trials_per_min", 0), t.get("mean_s", 0),
                     t.get("total_s", 0))
        fb = bm.get("final_build_s", {})
        if fb:
            log.info("  %-8s %-26s  OOF=%.1fs  Full=%.1fs  XGB=%.1fs  Physics=%.1fs",
                     f"{h}h", "Final Training",
                     bm.get("oof_generation_s", 0),
                     fb.get("lgbm_full", 0), fb.get("xgb", 0), fb.get("lgbm_physics", 0))
        log.info("  " + "-" * 72)
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--horizon", type=int, choices=[6, 12, 24, 48], default=None,
        metavar="{6,12,24,48}",
...
def main() -> None:
    ...
    log.info("V11 complete.  Total wall time: %.1fs (%.1f min)",
             total_run_s, total_run_s / 60)
