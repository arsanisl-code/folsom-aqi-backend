...
def fetch_airnow_current() -> dict | None:
    ...
    try:
        ...
        # Prefer the local station to avoid picking up distant stations (like Sacramento) with higher AQI
        local_data = [x for x in data if "Folsom" in x.get("ReportingArea", "")]
        if local_data:
